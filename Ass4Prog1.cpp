...
public:
    ...
    void withdrawbal(ACCOUNT b, double withdraw);
...
public:
    ...
    friend void BANK::addAccount(ACCOUNT b);
    friend void BANK::withdrawbal(ACCOUNT b, double withdraw);
};
...
void BANK::addAccount(ACCOUNT b)
{
        if (num >= 100)
            return;
        for (int i = 0; i < num; i++)
...
void BANK::withdrawbal(ACCOUNT b,double withdraw)
{
        for (int i = 0; i < num; i++)
        {
            if (b.getaccountid() == data[i].getaccountid())
            {
                if (data[i].getbalance() < withdraw)
                {
                    cout << "Withdraw greater than balance.";
                    return;
                }
                data[i].update(0.0, withdraw);
                return;
            }
        }
